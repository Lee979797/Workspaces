...
//-----------------------点击列表信息，显示图书的扩展属性-----------------------
var expander_zat = new Ext.grid.RowExpander({
...
//--------------------图片浏览窗口-------------------------------------------
var imageView_window_archivesTo = new Ext.Window({   
    id: 'image-window_7009',   
    title : '原文预览',   
    width : 900,   
    height : 600,   
    resizable : true, 
    maximizable:true,
    closeAction :'hide',
    html: '<OBJECT classid="clsid:B4E2D33E-D8B2-4956-9F91-1C6BDC87A6D1"   width=100%  height=100% align=center hspace=0 vspace=0 id="scanner7009"  name="scanner7009" >'
    ...
    buttons: [{   
	       text: '重载',   
		       handler: function() { 
		    	   var record = grid_archivesTo.getSelectionModel().getSelected();
		    		...
		    		Ext.Ajax.request({
						...
						success : function(result,request) {//获取返回值
							...
							if(resultObject!=null){
								scanner7009.OpenProgress(3);  //设置上传的进度条的总进度数
								scanner7009.Progress('原文加载中',1);
								scanner7009.Progress('原文加载中',2);
							    //resultObject = eval('('+result.responseText+')');  
			   					scanner7009.ImageData=resultObject.imageData;
		   						if(scanner7009.ImageData!=""){
		   							scanner7009.PageType=resultObject.pageTypeStr;
		   							scanner7009.Progress('原文加载完毕',3);
		   						}else{
		   							scanner7009.Progress('原文加载失败',3);
			   						alert("原文错误，加载失败，请重新扫描或导入！");
		   						}
		   						scanner7009.CloseProgress();
							}
						},
						failure : function() {
							alert("图像加载错误");
	        ...
	        text: '取消',   
	        handler: function() {   
	            imageView_window_archivesTo.hide();   
	        }
	   }],
	   listeners : {
			'activate' : function() {  //标签激活后自动加载数据
				...
				var useState=record.data.state;
				var	imgUrl='';
				scanner7009.ImageData="";
				scanner7009.PageType="";
...
//---------------------- 弹出浏览原文的窗口 ------------------------------------
function viewPic_archivesTo()
{
	imageView_window_archivesTo.show(); 
	imageView_window_archivesTo.setTitle('原文预览');
}
//---------------------- 列表显示原文的标识 ------------------------------------
function icon_archivesTo(value,p,record){
	if(typeof record=='object'){
		var fileUrl;
		fileUrl=record.data["imageData"];
		//alert(fileUrl);
		if(fileUrl!=''){
			return String.format('<a style="display:table;width:100%;" onclick="viewPic_archivesTo()"><img src="images/file_ico.gif"></a>',record.data["orgid"]);
		}
	}  
}
...
var cm_archivesTo = new Ext.grid.ColumnModel([expander_zat,
	sm2,
	{header : '原文',width : 20,dataIndex : 'imageUrl', renderer:icon_archivesTo,sortable : false},
...
var btn_return_archivesTo_create = new Ext.Button({
	text : '归档',
	iconCls : 'icon-store',
	disabled:true,
	handler : function(){
   				...
   				success: function(){
   					Ext.Msg.show({
						...
						fn : function(){
							//更新修改行的值
							record.set('state','5');
							grid_ArchivesTo.fireEvent('rowclick',grid_ArchivesTo,grid_ArchivesTo.getStore().indexOf(record));
							grid_ArchivesTo.getStore().remove(record);
...
//默认查询 limit为显示的条数
var searchArchivesTo = function() {
	ds_archivesTo.baseParams.conditions = text_search_archivesTo.getValue();
	ds_archivesTo.baseParams.username=currentZzUsername;
	ds_archivesTo.baseParams.stateConditions='4,14,24,34,44,54,64,74';
	ds_archivesTo.load({params : {start : 0,limit : 10} });

}

//查询返回结果的数据列
var ds_archivesTo = new Ext.data.Store({
...
var btn_search_archivesTo = new Ext.Button({
	text : '查询',
	iconCls : 'icon-search',
	handler : function() {
		searchArchivesTo();
		btn_return_archivesTo_create.setDisabled(true);
		//btn_return_archivesTo_code.setDisabled(true);
	}
});

var btn_refresh_archivesTo = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchArchivesToid.value='';
		searchArchives();
		btn_return_archivesTo_create.setDisabled(true);
		//btn_return_archivesTo_code.setDisabled(true);
	}
...
var text_search_archivesTo = new Ext.form.TextField({
	id:'textSearchArchivesToid',
	name : 'textSearchArchivesTo',
	width : 200,
...
//加载右栏主页面   （数据列表、工具栏按钮）
var grid_ArchivesTo = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	plugins : expander_zat,
	tbar : [btn_refresh_archivesTo,'-',btn_return_archivesTo_create,'->',text_search_archivesTo,btn_search_archivesTo],
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			...
			//btn_return_archivesTo_code.setDisabled(grid.getStore().getAt(rowIndex).data.state == 2 ? false:true);
			...
			{
				btn_return_archivesTo_create.setDisabled(false);	
			}
		}
	}
});
...
var zzArchivesTo_panel = new Ext.Panel({
	title : '待办管理',
	iconCls : 'icon-plugin',
	region : 'center',
	border : 'layout',
	...
	items: [{
	    region:'center',
	    layout : 'border',
	    items : [grid_ArchivesTo]
	}]
});

var p_zzArchivesTo = {
	id : 'zzArchivesTo-panel',
	border : false,
	layout : 'border',
	items : [zzArchivesTo_panel]
}
