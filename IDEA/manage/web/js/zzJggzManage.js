...
//-----------------------点击列表信息，显示图书的扩展属性-----------------------
var expander_jggzManage = new Ext.grid.RowExpander({
...
var cm_jggzManage = new Ext.grid.ColumnModel([expander_jggzManage,
	sm26785544,
	{header : '原文',width : 20,dataIndex : 'imageUrl', renderer:icon_jggzManage,sortable : false},
...
var jggzManageForm = new Ext.FormPanel({
	labelAlign : 'right',
	labelWidth : 90,
	...
	items : [
		...
		{xtype : 'textarea',fieldLabel : '搁置批复',name : 'auditNote',id:'auditNote_jggzManage',height:100,allowBlank:false}
	],
	buttonAlign : 'center',
	minButtonWidth : 60,
	buttons : [{
		...
		handler : function(btn){
			var record=grid_JggzManage.getSelectionModel().getSelected();
			if (record) {
				var strAuditNote=jggzManageForm.getForm().findField('auditNote_jggzManage').getValue();
				var strJgdm=record.data.jgdm;
				if (jggzManageForm.getForm().isValid()) {
					ajaxLoadMask.show();
					Ext.Ajax.request({
						url: 'returnTjgdmNo.action',
		   				params: { jgdm: strJgdm,auditNote:strAuditNote,memo3:'1'},
		   				success: function(response){
		   					window_gezhi_jggzManage.hide();
		   					ajaxLoadMask.hide();
		   					Ext.Msg.show({
								title : '提示',
								msg : '“搁置确认”操作成功!',
		...
		handler : function(btn){
			...
			if (record) {
				...
				if (jggzManageForm.getForm().isValid()) {
					...
					Ext.Ajax.request({
						url: 'returnTjgdmNo.action',
		   				params: { jgdm: strJgdm,auditNote:strAuditNote,memo3:'0'},
		   				success: function(response){
		   					window_gezhi_jggzManage.hide();
		   					ajaxLoadMask.hide();
		   					Ext.Msg.show({
...
var window_gezhi_jggzManage = new Ext.Window({
	title : '搁置批复',
	width : 500,
	resizable : false,
	autoHeight : true,
	...
	items : [jggzManageForm]
});


var btn_return_jggzManage = new Ext.Button({
	text : '搁置批复',
	iconCls : 'icon-do',
	disabled:true,
	handler : function(btn){
		var record=grid_JggzManage.getSelectionModel().getSelected();
		if (record) {
			window_gezhi_jggzManage.show();
			jggzManageForm.getForm().findField('jgdm').setValue(record.data.jgdm);
			jggzManageForm.getForm().findField('jgmc').setValue(record.data.jgmc);
			jggzManageForm.getForm().findField('handleNote').setValue(record.data.handleNote);
			btn.disable();
		}else{
			alert('请选择需搁置机构！');
		}
...
var btn_return_jggzManage_cancl = new Ext.Button({
	text : '确认取消搁置',
	iconCls : 'icon-ok',
	disabled:true,
	handler : function(btn){
		var record=grid_JggzManage.getSelectionModel().getSelected();
		if (record) {
			Ext.Msg.confirm('操作提示','确认“取消机构搁置”?',function(btn){
				if('yes' == btn){
					ajaxLoadMask.show();
					var strJgdm=record.data.jgdm;
					Ext.Ajax.request({
						url: 'returnTjgdmOk.action',
		   				params: { jgdm: strJgdm},
		   				success: function(response){
		   					ajaxLoadMask.hide();
		   					Ext.Msg.show({
...
//默认查询 limit为显示的条数
var searchJggzManage = function() {
	ds_jggzManage.baseParams.conditions = text_search_jggzManage.getValue();
	ds_jggzManage.baseParams.username=currentZzUsername;
	ds_jggzManage.baseParams.stateConditions='0,-100';
	ds_jggzManage.load({params : {start : 0,limit : useFullPageSize} });

}

//查询返回结果的数据列
var ds_jggzManage = new Ext.data.Store({
...
var btn_search_jggzManage = new Ext.Button({
	text : '查询',
	iconCls : 'icon-search',
	handler : function() {
		searchJggzManage();
		btn_return_jggzManage.setDisabled(true);
		btn_return_jggzManage_cancl.setDisabled(true);
	}
});

var btn_refresh_jggzManage = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchJggzManageid.value='';
		searchJggzManage();
		btn_return_jggzManage.setDisabled(true);
		btn_return_jggzManage_cancl.setDisabled(true);
	}
...
var text_search_jggzManage = new Ext.form.TextField({
	id:'textSearchJggzManageid',
	name : 'textSearchJggzManage',
	width : 200,
...
//--------------------图片浏览窗口-------------------------------------------
var imageView_window_jggzManage = new Ext.Window({   
    id: 'image-window_74832',   
    title : '原文预览',   
    width : 900,   
    height : 600,   
    resizable : true, 
    ...
    buttons: [{   
	       text: '重载',   
	        ...
	        text: '取消',   
	        handler: function() {   
	            imageView_window_jggzManage.hide();   
	        }
	   }],
	   listeners : {
...
//---------------------- 弹出浏览原文的窗口 ------------------------------------
function viewPic_jggzManage()
{
	imageView_window_jggzManage.show(); 
	imageView_window_jggzManage.setTitle('原文预览');
}
//---------------------- 列表显示原文的标识 ------------------------------------
function icon_jggzManage(value,p,record){
	if(typeof record=='object'){
		var fileUrl;
		fileUrl=record.data["imageData"];
		//alert(fileUrl);
		if(fileUrl!=''){
			return String.format('<a style="display:table;width:100%;" onclick="viewPic_jggzManage()"><img src="images/file_ico.gif"></a>',record.data["orgid"]);
		}
	}  
}

//加载右栏主页面   （数据列表、工具栏按钮）
var grid_JggzManage = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	plugins : expander_jggzManage,
	tbar : [btn_refresh_jggzManage,btn_return_jggzManage,btn_return_jggzManage_cancl,'->', 
		  	text_search_jggzManage,btn_search_jggzManage],
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			if(grid.getStore().getAt(rowIndex).data.state == '0' || grid.getStore().getAt(rowIndex).data.state == '100')
			{
				btn_return_jggzManage.setDisabled(false);
				btn_return_jggzManage_cancl.setDisabled(true);
			}
			if(grid.getStore().getAt(rowIndex).data.state == '-100')
			{
				btn_return_jggzManage.setDisabled(true);
				btn_return_jggzManage_cancl.setDisabled(false);
			}
		}
	}
});
...
var zzJggzManage_panel = new Ext.Panel({
	title : '机构搁置管理',
	iconCls : 'icon-plugin',
	region : 'center',
	border : 'layout',
	...
	items: [{
	    region:'center',
	    layout : 'border',
	    items : [grid_JggzManage]
	}]
});

var p_zzJggzManage = {
	id : 'zzJggzManage-panel',
	border : false,
	layout : 'border',
	items : [zzJggzManage_panel]
}