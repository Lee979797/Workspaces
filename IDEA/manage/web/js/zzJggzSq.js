...
//-----------------------点击列表信息，显示图书的扩展属性-----------------------
var expander_jggzsq = new Ext.grid.RowExpander({
...
var cm_jggzsq = new Ext.grid.ColumnModel([expander_jggzsq,
	sm26785544,
	{header : '原文',width : 20,dataIndex : 'imageUrl', renderer:icon_jggzsq,sortable : false},
	...
	{header : '办证机构代码',width : 60,sortable : true,dataIndex : 'bzjgdm'},
...
var jggzsqForm = new Ext.FormPanel({
	labelAlign : 'right',
	labelWidth : 70,
	...
	items : [
		...
		{xtype : 'textarea',fieldLabel : '搁置原因',name : 'handleNote',id:'handleNote_jggzsq',height:200,allowBlank:false}
	],
	buttonAlign : 'center',
	minButtonWidth : 60,
	buttons : [{
		...
		handler : function(btn){
			var record=grid_JggzSq.getSelectionModel().getSelected();
			if (record) {
				var strHandleNote=jggzsqForm.getForm().findField('handleNote_jggzsq').getValue();
				var strJgdm=record.data.jgdm;
				if (jggzsqForm.getForm().isValid()) {
					ajaxLoadMask.show();
					Ext.Ajax.request({
						url: 'returnTjgdmGzsq.action',
		   				params: { jgdm: strJgdm,handleNote:strHandleNote},
		   				success: function(response){
		   					window_gezhi_jggzsq.hide();
		   					ajaxLoadMask.hide();
		   					Ext.Msg.show({
								title : '提示',
								msg : '“搁置申请”操作成功!',
...
var window_gezhi_jggzsq = new Ext.Window({
	title : '搁置申请',
	width : 400,
	resizable : false,
	autoHeight : true,
...
var btn_return_jggzsq = new Ext.Button({
	text : '搁置申请',
	iconCls : 'icon-add',
	disabled:true,
	handler : function(btn){
		var record=grid_JggzSq.getSelectionModel().getSelected();
		if (record) {
			window_gezhi_jggzsq.show();
			jggzsqForm.getForm().findField('jgdm').setValue(record.data.jgdm);
			jggzsqForm.getForm().findField('jgmc').setValue(record.data.jgmc);
			btn.disable();
		}else{
			alert('请选择需搁置机构！');
		}
...
var btn_return_jggzsq_cancl = new Ext.Button({
	text : '删除搁置申请',
	iconCls : 'icon-del',
	disabled:true,
	handler : function(btn){
...
//默认查询 limit为显示的条数
var searchJggz = function() {
	ds_jggzsq.baseParams.conditions = text_search_jggzsq.getValue();
	ds_jggzsq.baseParams.username=currentZzUsername;
	ds_jggzsq.baseParams.userBzjgdm=currentZzUserBzjgdm;
	//ds_jggzsq.baseParams.stateConditions='100,0';
	ds_jggzsq.load({params : {start : 0,limit : useFullPageSize} });

}

//查询返回结果的数据列
var ds_jggzsq = new Ext.data.Store({
...
var btn_search_jggzsq = new Ext.Button({
	text : '查询',
	iconCls : 'icon-search',
	handler : function() {
		searchJggz();
		btn_return_jggzsq.setDisabled(true);
		btn_return_jggzsq_cancl.setDisabled(true);
	}
});

var btn_refresh_jggzsq = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchJggzid.value='';
		searchJggz();
		btn_return_jggzsq.setDisabled(true);
		btn_return_jggzsq_cancl.setDisabled(true);
	}
...
var text_search_jggzsq = new Ext.form.TextField({
	id:'textSearchJggzid',
	name : 'textSearchJggz',
	width : 200,
...
//--------------------图片浏览窗口-------------------------------------------
var imageView_window_jggzsq = new Ext.Window({   
    ...
    maximizable:true,
    closeAction :'hide',
    html: '<OBJECT classid="clsid:B4E2D33E-D8B2-4956-9F91-1C6BDC87A6D1"   width=100%  height=100% align=center hspace=0 vspace=0 id="scanner70832"  name="scanner70832" >'
    ...
    buttons: [{   
		    		...
		    		Ext.Ajax.request({
						...
						success : function(result,request) {//获取返回值
							...
							if(resultObject!=null){
								scanner70832.OpenProgress(3);  //设置上传的进度条的总进度数
								scanner70832.Progress('原文加载中',1);
								scanner70832.Progress('原文加载中',2);
							    //resultObject = eval('('+result.responseText+')');  
			   					scanner70832.ImageData=resultObject.imageData;
		   						if(scanner70832.ImageData!=""){
		   							scanner70832.PageType=resultObject.pageTypeStr;
		   							scanner70832.Progress('原文加载完毕',3);
		   						}else{
		   							scanner70832.Progress('原文加载失败',3);
			   						alert("原文错误，加载失败，请重新扫描或导入！");
		   						}
		   						scanner70832.CloseProgress();
							}
						},
						failure : function() {
							alert("图像加载错误");
	   ...
	   listeners : {
			'activate' : function() {  //标签激活后自动加载数据
				...
				var useState=record.data.state;
				var	imgUrl='';
				scanner70832.ImageData="";
				scanner70832.PageType="";
				imgUrl='orgnewViewImg.action';
	    		Ext.Ajax.request({
					url : imgUrl,
					params : {orgid : record.data.orgid},
...
//---------------------- 弹出浏览原文的窗口 ------------------------------------
function viewPic_jggzsq()
{
	imageView_window_jggzsq.show(); 
	imageView_window_jggzsq.setTitle('原文预览');
}
//---------------------- 列表显示原文的标识 ------------------------------------
function icon_jggzsq(value,p,record){
	if(typeof record=='object'){
		var fileUrl;
		fileUrl=record.data["imageData"];
		//alert(fileUrl);
		if(fileUrl!=''){
			return String.format('<a style="display:table;width:100%;" onclick="viewPic_jggzsq()"><img src="images/file_ico.gif"></a>',record.data["orgid"]);
		}
	}  
}
...
//加载右栏主页面   （数据列表、工具栏按钮）
var grid_JggzSq = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	plugins : expander_jggzsq,
	tbar : [btn_refresh_jggzsq,btn_return_jggzsq,btn_return_jggzsq_cancl,'->', 
		  	text_search_jggzsq,btn_search_jggzsq],
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			if(grid.getStore().getAt(rowIndex).data.state == '100' || grid.getStore().getAt(rowIndex).data.state == '')
			{
				btn_return_jggzsq.setDisabled(false);
				btn_return_jggzsq_cancl.setDisabled(true);
			}
			
			if(grid.getStore().getAt(rowIndex).data.state == '0' )
			{
				btn_return_jggzsq.setDisabled(true);
				btn_return_jggzsq_cancl.setDisabled(false);
			}
		}
	}
});
...
var zzJggzSq_panel = new Ext.Panel({
	title : '机构搁置申请',
	iconCls : 'icon-plugin',
	region : 'center',
	border : 'layout',
