...
//-----------------------点击列表信息，显示图书的扩展属性-----------------------
var expander_zwq = new Ext.grid.RowExpander({
...
//--------------------图片浏览窗口-------------------------------------------
var imageView_window_workQuery = new Ext.Window({   
    id: 'image-window_701119',   
    title : '原文预览',   
    width : 900,   
    height : 600,   
    resizable : true, 
    maximizable:true,
    closeAction :'hide',
    html: '<OBJECT classid="clsid:B4E2D33E-D8B2-4956-9F91-1C6BDC87A6D1"   width=100%  height=100% align=center hspace=0 vspace=0 id="scanner70011"  name="scanner70011" >'
    ...
    buttons: [{   
	       text: '重载',   
		       handler: function() { 
		    	   var record = grid_WorkQuery.getSelectionModel().getSelected();
		    		...
		    		Ext.Ajax.request({
						...
						success : function(result,request) {//获取返回值
							...
							if(resultObject!=null){
								scanner70011.OpenProgress(3);  //设置上传的进度条的总进度数
								scanner70011.Progress('原文加载中',1);
								scanner70011.Progress('原文加载中',2);
							    //resultObject = eval('('+result.responseText+')');  
			   					scanner70011.ImageData=resultObject.imageData;
		   						if(scanner70011.ImageData!=""){
		   							scanner70011.PageType=resultObject.pageTypeStr;
		   							scanner70011.Progress('原文加载完毕',3);
		   						}else{
		   							scanner70011.Progress('原文加载失败',3);
			   						alert("原文错误，加载失败，请重新扫描或导入！");
		   						}
		   						scanner70011.CloseProgress();
							}
						},
						failure : function() {
							alert("图像加载错误");
	        ...
	        text: '取消',   
	        handler: function() {   
	            imageView_window_workQuery.hide();   
	        }
	   }],
	   listeners : {
...
//---------------------- 弹出浏览原文的窗口 ------------------------------------
function viewPic_workQuery()
{
	imageView_window_workQuery.show(); 
	imageView_window_workQuery.setTitle('原文预览');
}
//---------------------- 列表显示原文的标识 ------------------------------------
function icon_workQuery(value,p,record){
	if(typeof record=='object'){
		var fileUrl;
		fileUrl=record.data["imageData"];
		//alert(fileUrl);
		if(fileUrl!=''){
			return String.format('<a style="display:table;width:100%;" onclick="viewPic_workQuery()"><img src="images/file_ico.gif"></a>',record.data["orgid"]);
		}
	}  
}
...
//---------------------- 列表显示主要数据信息 ----------------------------------
// 列表显示机构的主要信息 yangqi
var sm1122 = new Ext.grid.CheckboxSelectionModel();
var cm_workQuery = new Ext.grid.ColumnModel([expander_zwq,
	sm1122,
	{header : '原文',width : 20,dataIndex : 'imageUrl', renderer:icon_workQuery,sortable : false},
	...
	{header : '机构类型',width : 70,sortable : true,dataIndex : 'jglx'},
	{header : '业务类型',width : 50,dataIndex : 'ywlx',sortable : true},
	{header : '办理人',width : 50,dataIndex : 'handleName',id : 'handleName',sortable : true},
	{header : '审核人',	width : 50,dataIndex : 'auditName',id : 'auditName',sortable : true},
	{header : '审核意见',width : 100,dataIndex : 'auditNote',id : 'auditNote',sortable : true},
	{header : '状态',width : 60,dataIndex : 'state',renderer : goState,menuDisabled : true}
]);


var btn_del_workQuery = new Ext.Button({
	text : '删除',
	iconCls : 'icon-del',
	disabled:true,
	handler : function() {
			...
			Ext.Msg.confirm('确认删除', '你确定删除该件工单?', function(btn) {
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
						...
						params : {orgid : record.data.orgid},
						success : function() {ajaxLoadMask.hide();grid_WorkQuery.getStore().remove(record);},
						failure : function() {
							ajaxLoadMask.hide();
							Ext.Msg.show({
								title : '错误提示',
...
//默认查询 limit为显示的条数
var searchWorkQuery = function() {
	ds_workQuery.baseParams.conditions = text_search_workQuery.getValue();
	ds_workQuery.baseParams.username=currentZzUsername;
	//ds_workQuery.baseParams.ywlx=(currentZzUserBzjgdm==currentZzUserCenterid?"":null);
	ds_workQuery.baseParams.stateConditions='12,13,14,15,22';
	ds_workQuery.load({params : {start : 0,limit : useFullPageSize} });

}

//查询返回结果的数据列
var ds_workQuery = new Ext.data.Store({
...
var btn_search_workQuery = new Ext.Button({
	text : '查询',
	iconCls : 'icon-search',
	handler : function() {
		searchWorkQuery();
...
var btn_refresh_workQuery = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchWorkQueryid.value='';
		searchWorkQuery();
	}
});
...
var text_search_workQuery = new Ext.form.TextField({
	id:'textSearchWorkQueryid',
	name : 'textSearchWorkQuery',
	width : 200,
...
//加载右栏主页面   （数据列表、工具栏按钮）
var grid_WorkQuery = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	autoExpandColumn : 'auditNote',
	viewConfig : {forceFit : true},
	plugins : expander_zwq,
	tbar : [/*btn_del_workQuery,*/btn_refresh_workQuery,'->',text_search_workQuery,btn_search_workQuery],
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			if(grid.getStore().getAt(rowIndex).data.state == 12 || grid.getStore().getAt(rowIndex).data.state == 13 || grid.getStore().getAt(rowIndex).data.state == 14)
			{
				btn_del_workQuery.setDisabled(false);
			}else{
				btn_del_workQuery.setDisabled(true);
			}
		}
	}
});
...
var zzWorkQuery_panel = new Ext.Panel({
	title : '待办查询',
	iconCls : 'icon-plugin',
	region : 'center',
	border : 'layout',
	...
	items: [{
	    region:'center',
	    layout : 'border',
	    items : [grid_WorkQuery]
	}]
});

var p_zzWorkQuery = {
	id : 'zzWorkQuery-panel',
	border : false,
	layout : 'border',
	items : [zzWorkQuery_panel]
}