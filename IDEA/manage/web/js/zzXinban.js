var searchXzqh_xinban2 = function() {
	ds_xzqh_xinban2.baseParams.conditions = text_search_xzqh_xinban2.getValue();
	ds_xzqh_xinban2.baseParams.username='';
	ds_xzqh_xinban2.baseParams.stateConditions='';
	ds_xzqh_xinban2.load({params : {start : 0,limit : 20} });
}

var btn_search_xzqh_xinban2 = new Ext.Button({
	text : '查询',
	iconCls : 'icon-search',
	handler : function() {
		searchXzqh_xinban2();
...
var text_search_xzqh_xinban2 = new Ext.form.TextField({
	id : 'textSearchXzqh_xinban2',
	name : 'textSearchXzqh_xinban2',
	width : 200,
...
var grid_xzqh_xinban2 = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	tbar : ['请输入行政区划名称：',text_search_xzqh_xinban2,btn_search_xzqh_xinban2],
	bbar : new Ext.PagingToolbar({
		pageSize : 20,
		store : ds_xzqh_xinban2,
		displayInfo : true,
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			window_xzqhQuery_xinban2.hide();
			var selections=grid.getSelectionModel().getSelections();
			xinbanViewForm.getForm().findField('xzqhName_xinban2').setValue(selections[0].get('xzqhName'));
			xinbanViewForm.getForm().findField('xzqhCode_xinban2').setValue(selections[0].get('xzqhCode'));
		}
	}
});

var window_xzqhQuery_xinban2 = new Ext.Window({
	title : '行政区划名称查询',
	iconCls : 'icon-plugin',
	width :500,
	height:400,
...
var grid_jjlx_xinban = new Ext.grid.GridPanel({
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			...
			xinbanViewForm.getForm().findField('select_jjlxmcOld_xinban').setValue(selections[0].get('ojjlxmc00'));
			xinbanViewForm.getForm().findField('select_jjlxdmOld_xinban').setValue(selections[0].get('ojjlxdm00'));
...
var searchZgmc_xb = function() {
	...
	ds_zgmc_xb.baseParams.userBzjgdm=null;
...
var grid_zgmc_xb = new Ext.grid.GridPanel({
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			...
			var selections=grid.getSelectionModel().getSelections();
			xinbanViewForm.getForm().findField('select_zgmc_xinban').setValue(selections[0].get('jgmc'));
...
var xinbanViewForm = new Ext.FormPanel({	
	...
	items:[{
            ...
            items:[{
                ...
                border:false,
                items: [{
                	xtype:'textfield',fieldLabel : '移动电话',id : 'mobile_xb',
                	name : 'mobile',labelSeparator:'',vtype:'mobilephone',
                	maxLength:11,minLength:11,anchor:'100%'
                	,enableKeyEvents:true,
            		listeners: {
            			keyup: function(f,e){
            				xinbanViewForm.findById('mobile_xinban').setText("("+f.getValue().length+")");
            			}
            		}
                }]
			},{
				...
				baseCls : 'x-plain',
				defaultType : 'label',
				items : [{id:'mobile_xinban',name:'mobile_xinban',text:'(0)'}]
		    },{
                columnWidth:.95,
                layout: 'form',
                bodyStyle: 'padding:0px',
                ...
                items: [{
			    	...
			    	listeners:{
			    		keyup: function(f, e){//计数
				    		var length = 1000-f.getValue().length
				    		xinbanViewForm.findById('jj_xb').setText("("+length+")");
				    	}
			    	}
                }]
			},{
				...
				baseCls : 'x-plain',
				defaultType : 'label',
				items : [{id:'jj_xb',name:'jj_xb',text:'(1000)'}]
			},{
				columnWidth:.5,
                layout: 'form',
                bodyStyle: 'padding:0px',
                ...
                items: [new Ext.form.TriggerField({
	                ...
	                layout: 'form',
	                border:false,
	                items: [{fieldLabel : '注册资金(万)',name : 'zczj',labelSeparator:'',allowBlank : false,
	                	xtype : 'numberfield',decimalPrecision :6,maxValue : 2000000000,anchor:'97%'
	                }]
	        },{
	                columnWidth:.2,
	                layout: 'form',
	                ...
	                items: [{ xtype:'textfield',fieldLabel : '注册号',name : 'zch',labelSeparator:'',allowBlank : false,maxLength : 50,anchor:'100%',
							enableKeyEvents:true,
			                		listeners: {
			                			keyup: function(f,e){
			                				xinbanViewForm.findById('zch_xb').setText("("+f.getValue().length+")");
			                			}
			                		}
					}]
			    },{
					...
					baseCls : 'x-plain',
					defaultType : 'label',
					items : [{id:'zch_xb',name:'zch_xb',text:'(0)'}]
				},{
	                columnWidth:.45,
	                layout: 'form',
	                border:false,
	                items: [{ 
	            		...
	            		listeners: {
	            			keyup: function(f,e){
	            				xinbanViewForm.findById('dhhm_xinban').setText("("+f.getValue().length+")");
	            			}
	            		}
	                }]
	            },{
					...
					baseCls : 'x-plain',
					defaultType : 'label',
					items : [{id:'dhhm_xinban',name:'dhhm_xinban',text:'(0)'}]
			    },{
	                columnWidth:1,
	                layout: 'form',
	                border:false,
	                items: [
	                	{ xtype:'textfield',fieldLabel : '机构注册地址',name : 'jgdz',labelSeparator:'',allowBlank : false,maxLength : 100,anchor:'99.9%',value:'湖北省武汉市'}
	                ]
	            },{
	                columnWidth:.25,
	                layout: 'form',
	                ...
	                items: [
	                	new Ext.form.TriggerField({
						    ...
						    listeners : {
								'specialkey' : function(field, e) {
									if (e.getKey() == Ext.EventObject.ENTER) {
										findXzqhNameByXzqhCode(field,'xzqhName_xinban');
									}
								},
								'blur': function(field){
									findXzqhNameByXzqhCode(field,'xzqhName_xinban');
	                ...
	                items: [
	                	new Ext.form.TriggerField({
						    ...
						    editable : false,
						    onTriggerClick : function() {
						    	window_xzqhQuery_xinban2.show();
						    },
						    listeners : {
								'specialkey' : function(field, e) {
									if (e.getKey() == Ext.EventObject.ENTER) {
										findXzqhNameByXzqhCode(field,'xzqhName_xinban2');
									}
								},
								'blur': function(field){
									findXzqhNameByXzqhCode(field,'xzqhName_xinban2');
	                ...
	                layout: 'form',
	                border:false,
	                items: [{ xtype:'textfield',fieldLabel : '行政区划代码',id:'xzqhName_xinban2',name : 'xzqhName2',labelSeparator:'',hideLabel:true,readOnly:true,maxLength : 50,anchor:'99.9%'}]
	            },{
	                columnWidth:.25,
	                layout: 'form',
	                border:false,
				...
				},
				{
	                ...
	                items: [{ 
	            		...
	            		listeners: {
	            			keyup: function(f,e){
	            				xinbanViewForm.findById('tbrlxfs_xinban').setText("("+f.getValue().length+")");
	            			}
	            		}
	                }]
				},{
					...
					baseCls : 'x-plain',
					defaultType : 'label',
					items : [{id:'tbrlxfs_xinban',name:'tbrlxfs_xinban',text:'(0)'}]
			    },{
	                columnWidth:.33,
	                layout: 'form',
	                bodyStyle: 'padding:0px',
...
var searchXinban = function() {
	Ext.getCmp("jgmc_xinban").getEl().dom.readOnly = false;
	...
	xinbanViewForm.getForm().load({
	    ...
	    params: { conditions:  text_search_xinban.getValue(),username:currentZzUsername,ywlxConditions:null,stateConditions:'12,13,14,15,16',start:1,limit:1},	
	    success:function(form,action) {//获取返回值action.result.data.state=='10' || 
			    	...
			    	if(action.result.data.ywlx=='新办' && (action.result.data.state=='12' || action.result.data.state=='14' || action.result.data.state=='15')){   //判断业务是否正在办理中
			    		...
			    		*/
			    		...
			    		if(xinbanViewForm.getForm().findField('bzrq').getValue()==null||xinbanViewForm.getForm().findField('zfrq').getValue()==null){
			    			xinbanViewForm.getForm().findField('bzrq').setValue(myDate.format('Y-m-d'));
			    			xinbanViewForm.getForm().findField('zfrq').setValue(newZfrqYes);
			    		}
						...
						if(action.result.data.zbsl == null || action.result.data.zbsl == ''){
				    		xinbanViewForm.getForm().findField('zbsl').setValue(1);
			    		}
			    		xinbanViewForm.getForm().findField('bzjgdm').setValue(currentZzUserBzjgdm);
		...
		failure : function() {//正式库检索
				...
				xinbanViewForm.getForm().load({
				    ...
				    method: 'POST', //为"POST",则上面的url不会显示在浏览器的地址栏中
				    params: { jgdm:  text_search_xinban.getValue(),ywlx:'预赋码',userBzjgdm:null},	
				    success:function(form,action) {//获取返回值
					    	if(currentZzUsername!='admin' && sysBzjgLimitMode=='1' && action.result.data.bzjgdm!=currentZzUserBzjgdm && currentZzUserYwqx!='2' && currentZzUserBzjgdm!=currentZzUserCenterid){
				    			...
				    			if(action.result.data.state=='100' || action.result.data.state==''){ //判断此机构状态是否正常
								    	if(action.result.data.orgid!='' && action.result.data.ywlx=='预赋码'){//自动加载原文
									    		...
									    		xinbanViewForm.getForm().findField('state').setValue('12');
									    		xinbanViewForm.getForm().findField('orgid').setValue('');
									    		xinbanViewForm.getForm().findField('scbzrq').setValue(myDate.format('Y-m-d'));
									    		xinbanViewForm.getForm().findField('bzrq').setValue(myDate.format('Y-m-d'));
									    		xinbanViewForm.getForm().findField('lastdate').setValue(myDate.format('Y-m-d'));
...
var btn_del_xinban = new Ext.Button({
	...
	disabled:true,
	handler : function() {
		var orgid = xinbanViewForm.getForm().findField('orgid').getValue();
		if(orgid!=null && orgid!=""){
	...
	buttons : [{
		...
		handler : function(btn){
			if(xinbanFumaForm.getForm().findField('dmlx_xinban').getValue()!=""){
				var record = grid_code_creat_xinban.getStore().getAt(0);
				
				btn.setText('取码');
				ds_code_creat_xb.removeAll();
				ds_code_creat_xb.baseParams.zch = xinbanViewForm.getForm().findField('zch').getValue();
				ds_code_creat_xb.baseParams.orgid = xinbanViewForm.getForm().findField('orgid').getValue();
				ds_code_creat_xb.baseParams.state = xinbanViewForm.getForm().findField('state').getValue();
				ds_code_creat_xb.baseParams.jgmc = xinbanViewForm.getForm().findField('jgmc').getValue();
				ds_code_creat_xb.baseParams.jgdz = xinbanViewForm.getForm().findField('jgdz').getValue();
				ds_code_creat_xb.baseParams.centerid=xinbanViewForm.getForm().findField('centerid').getValue();
					...
					callback: function(records, options, success){
						if(records!=null){
							if(records[0].data.flag=='1'|| records[0].data.flag=='3'){
								...
								btn.disable();
								var btnTrueXBfuma=Ext.getCmp("btn_true_xbfuma");
								btnTrueXBfuma.enable();
								var btnFalseXBfuma=Ext.getCmp("btn_false_xbfuma");
								btnFalseXBfuma.disable();
							}else{
								btn.disable();
								var btnTrueXBfuma=Ext.getCmp("btn_true_xbfuma");
								btnTrueXBfuma.disable();
								var btnFalseXBfuma=Ext.getCmp("btn_false_xbfuma");
								btnFalseXBfuma.enable();
							}
						}
					}
				});
			...
			if(xinbanFumaForm.getForm().isValid()){
				xinbanFumaForm.getForm().submit({
					...
					success : function(form) {
						xinbanViewForm.getForm().findField('jgdm').setValue(xinbanFumaForm.getForm().findField('jgdm').getValue());
						ds_code_creat_xb.removeAll();
						window_fuma_xb.hide();
						var tab34529=Ext.getCmp("jbInfo21");
					    tab34529.setTitle("基本信息(赋码成功，请提交确认)");
					    btn_code_xinban.setDisabled(true);
					    Ext.getCmp("jgmc_xinban").getEl().dom.readOnly = true;
					},
...
var ds_code_creat_xb=new Ext.data.Store({
...
var cm_code_creat_xb = new Ext.grid.ColumnModel([
   {header : '机构代码',width:80,dataIndex : 'jgdm',sortable : true}, 
   {header : '机构名称',width:160,dataIndex : 'jgmc',id:'jgmc_xb_code',sortable : true}, 
...
xinbanCodeListPanel = Ext.extend(Ext.grid.GridPanel,{
	constructor:function(){
		xinbanCodeListPanel.superclass.constructor.call(this,grid_code_creat_xinban);
	}
});

var grid_code_creat_xinban = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	enableColumnMove : false,
	cm : cm_code_creat_xb,
	ds : ds_code_creat_xb,
	sm : new Ext.grid.RowSelectionModel({singleSelect : true}),
	autoExpandColumn : 'jgmc_xb_code'/*,
	listeners : {
		'rowclick':function(grid,rowIndex){
			var btn345291=Ext.getCmp("btn_qm_xb");
			if(grid.getStore().getAt(rowIndex).data.flag=='1' || grid.getStore().getAt(rowIndex).data.flag=='3'){
				xinbanFumaForm.getForm().findField('jgdm').setValue(grid.getStore().getAt(rowIndex).data.jgdm);
			 	btn345291.disable();
    		}else{
    			if(grid.getStore().getAt(rowIndex).data.flag=='4'){
			 		btn345291.setText("重新取码");
    			}
    		}
		}
	}*/
...
var window_fuma_xb = new Ext.Window({
	...
	listeners : {
		/*'hide' : function(){
			xinbanFumaForm.getForm().reset();
			var btn345296781123=Ext.getCmp("btn_qm_xb");
			btn345296781123.setText("取码");
			btn345296781123.disable();
		},*/
		'show' : function(){
			xinbanFumaForm.getForm().reset();
			var btnXBfuma=Ext.getCmp("btn_qm_xb");
			btnXBfuma.enable();
			var btnTrueXBfuma=Ext.getCmp("btn_true_xbfuma");
			btnTrueXBfuma.disable();
			var btnFalseXBfuma=Ext.getCmp("btn_false_xbfuma");
...
var btn_code_xinban = new Ext.Button({
	...
	handler : function(){
		window_fuma_xb.show();
		xinbanFumaForm.getForm().findField('orgid').setValue(xinbanViewForm.getForm().findField('orgid').getValue());
		xinbanFumaForm.getForm().findField('jgmc').setValue(xinbanViewForm.getForm().findField('jgmc').getValue());
		xinbanFumaForm.getForm().findField('jgdz').setValue(xinbanViewForm.getForm().findField('jgdz').getValue());
		xinbanFumaForm.getForm().findField('zch').setValue(xinbanViewForm.getForm().findField('zch').getValue());
		var jglxdm = xinbanViewForm.getForm().findField('jglxdm').getValue();
		//alert(jglxdm);
		if(jglxdm=='98'){
			xinbanFumaForm.getForm().findField('dmlx_xinban').setValue('0');
		}else{
			xinbanFumaForm.getForm().findField('dmlx_xinban').setValue('3');
		}
	}
});
...
var upFuncXB = function(docid,orgid,strJgdm,strState,strYwlx,strJgmc){
	//数据保存成功后，再自动保存原文
				var base64file;
				var packLength;
				var packCount;
				...
				strDocid = docid;
				strBzjgdm=xinbanViewForm.getForm().findField('bzjgdm').getValue(); 
				...
				imageCount = scanner24622.ImageCount;	//获取扫描图片的页数
				...
				Ext.Msg.show({
					...
					fn : function() {
						...
						//alert(sysStrYwxcSet);
						if(sysXcWorkMode=='0'){
							btn_code_xinban.setDisabled(false);
						}
						scanner24622.CloseProgress();
						
						subXBAction(orgid,strJgdm,strState,strYwlx,strJgmc);
					},
					icon : Ext.Msg.INFO
				});
...
function btn_xinban_saveAll(btn){
	...
	if (xinbanViewForm.getForm().isValid()) {
		var strdocid = xinbanViewForm.getForm().findField('docid').getValue();
		var strjgmc = xinbanViewForm.getForm().findField('jgmc').getValue();
		if(!validateJgmc(strjgmc,strdocid)){
			return
		}
		if(xinbanViewForm.getForm().findField('select_jglxdm_xinban').getValue()=='11'&&xinbanViewForm.getForm().findField('zczj').getValue()<=0){
			alert('注册资金不能低于0');
			return
		}
		xinbanViewForm.getForm().submit({
			...
			success : function(form,action) {
				...
				var strState = xinbanViewForm.getForm().findField('state').getValue();
				...
				var strYwlx = xinbanViewForm.getForm().findField('ywlx').getValue();
				//alert(strState+'  '+(strState=="15" && strJgdm==""));
				if(scanner24622.ImageCount>0){
					if(orgid!=null && orgid!=""){
						if(strState=="15" && strJgdm==""){
							...
							Ext.Ajax.request({
								...
								success: function(result,request){//result,request
									...
									var str = scanner24622.CheckArchives("申请",data.jglxdmOld,data.zchInfo,data.frInfo);
									if(str==''){
										//alert("更新imageFlag为1（表示档案完整）");
										refreshImageFlag(orgid,'1');
										upFuncXB(docid,orgid,strJgdm,strState,strYwlx,strJgmc);
									}else{
...
function btn_xinban_ftpFile(btn){
	...
	btn.disable();
	strOrgid =xinbanViewForm.getForm().findField('orgid').getValue();  //参数orgid
	strDocid =xinbanViewForm.getForm().findField('docid').getValue();  //参数docid
	strBzjgdm=xinbanViewForm.getForm().findField('bzjgdm').getValue();  //办证机构代码
	...
	if(strOrgid!=""){
			...
			Ext.Msg.show({
				...
				buttons : Ext.Msg.OK,
				fn : function() {btn.enable();btn_code_xinban.setDisabled(false);},
				icon : Ext.Msg.INFO
			});
...
var btn_saveInfo_xinban = new Ext.Button({
	text : '保存数据',
	iconCls: 'blist',
	handler : btn_xinban_saveInfo
});

var btn_print_xinban = new Ext.Button({
	text : '打印申请表',
	iconCls : 'icon-print',
...
//提交确认方法
var subXBAction =function (orgid,strJgdm,strState,strYwlx,strJgmc){
	...
	Ext.Ajax.request({
		...
		success: function(result,request){
			...
			if(data.state=='15'){
				//根据现场流程参数设置，来判断赋码状态
			    btn_code_xinban.setDisabled(isFumaBtnActive(xinbanViewForm.getForm().findField('jgdm').getValue(),data.state,'新办',sysXcWorkMode,sysStrYwxcSet)); 
				xinbanViewForm.getForm().findField('state').setValue(data.state);
				xinbanViewForm.getForm().findField('jgdm_xinban').allowBlank=false;
			}else{
				Ext.Msg.show({
					title : '提示',
...
var p_zzXinban = {
	...
	items : [{
	    ...
	    iconCls : 'icon-plugin',
 		plain: true,
		region: 'center',
		tbar : [btn_shibie_xinban,btn_refresh_xinban,btn_code_xinban,btn_del_xinban,btn_print_xinban/*,btn_saveInfo_xinbanbtn_save_xinban_menu*/,btn_ok_xinban,'->',text_search_xinban,btn_search_xinban],
