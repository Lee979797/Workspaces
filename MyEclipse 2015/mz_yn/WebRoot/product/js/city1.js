...
function GetTwoXzqh1(option,twoXzqh){
	...
	cityObj.options.length = 0;
	dwr.engine.setAsync(false);
	ZrxzqhBus.getSecondGradeElement(dmId,{callback:function(citys){
				cityObj.options.add(new Option("-- 选择 --",""));
				if(citys != null && citys.length != 0){
...
function GetThreeXzqh1(option,threeXzqh){
	...
	cityObj.options.length = 0;
	dwr.engine.setAsync(false);
	ZrxzqhBus.getThirdGradeElement(dmId,{callback:function(citys){
		cityObj.options.add(new Option("-- 选择 --",""));
				if(citys != null && citys.length != 0){
					cityObj.removeAttribute('disabled');
					for(var i=0;i<citys.length;i++){
