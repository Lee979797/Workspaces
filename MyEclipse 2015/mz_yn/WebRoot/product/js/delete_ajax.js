...
xmlhttp.onreadystatechange = function() {   
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {   				
				if(xmlhttp.responseText.trim().indexOf("success") > -1){
					...
					ymPrompt.errorInfo("删除失败！",200,150,"提示信息");
				}
...
xmlhttp.onreadystatechange = function() {   
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {   
				...
				if(xmlhttp.responseText.trim().indexOf("success") > -1){
					ymPrompt.succeedInfo('同步成功！',330,220,'提示信息',function(data){if(data == "ok"){window.location.reload();}});
					
				}else{
					ymPrompt.errorInfo("同步失败！",200,150,"提示信息");
				}
...
xmlhttp.onreadystatechange = function() {   
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {   
				...
				if(xmlhttp.responseText.trim().indexOf("success") > -1){
					ymPrompt.succeedInfo('操作成功！',330,220,'提示信息',function(data){if(data == "ok"){window.location.reload();}});
					
				}else{
					ymPrompt.errorInfo("操作失败！",200,150,"提示信息");
				}
...
xmlhttp.onreadystatechange = function() {   
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {   
				...
				if(xmlhttp.responseText.trim().indexOf("delGroupSuc") > -1){
					ymPrompt.succeedInfo('用户组删除成功！',330,220,'提示信息',function(data){if(data == "ok"){window.location.reload();}});
