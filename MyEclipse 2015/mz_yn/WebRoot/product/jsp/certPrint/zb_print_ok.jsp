...
<head>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function load() {
            if (fbNum > 1) {
                var flag = confirm("输入副本编号吗？共需要输入'" + fbNum + "'次，取消后，可以在证书修改页填写证书编号");
                print_djh("${fn:trim(certi.djhFbPre)}", 1, start + 1, flag, false)
            } else if (fbNum > 0) {
                print_djh("${fn:trim(certi.djhFbPre)}", 1, start + 1, true, false)
            } else {
            ...
            if (tp != 'close' && tp != '') {
                ...
                if (index <= (start + fbNum)) {
                    ...
                    if (Number('${certi.zbNumber}') > 0
                            && (again && again == "false")) {
                        //打印完如果是再次打印，不执行打印正本
                        print_djh("${fn:trim(certi.djh)}", 0, "", true, again);
                    }
                }
        ...
        function printAgain(flag) {//再次打印
            ...
            if (flag == "0") {
                print_djh("${fn:trim(certi.djh)}", 0, "", true, true)
            }
            if (flag == "1") {
                if (fbNum > 1) {
                    if (force && force == true) {
