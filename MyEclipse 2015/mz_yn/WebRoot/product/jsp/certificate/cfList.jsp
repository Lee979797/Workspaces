...
<body>
<div class="page_top" style="text-align:center;font-weight:bold;">${type==1?'注册号':(type==2?'机构名称':'法定代表人/负责人')}重复检测情况</div>
<div id="list_main">
  <div class="list_box">
      		...
      		<tr ${i.count % 2==1?"class='list_tr'":"class='list_tr2'" }>
                ...
                <c:choose>
                    <c:when test="${tjgdmsave.tableName eq 't_jgdm_save' }">
                        <td align="center"><img src="../../images/icon_chakan.gif" width="16" height="16" onclick="window.open('/bsweb/certificate_viewFddbrPage.action?id=${tjgdmsave.id}&sourceTable=${tjgdmsave.tableName}')" style="cursor:pointer" title="查看"/></td>
                    </c:when>
                    <c:when test="${fn:contains(tjgdmsave.sjly,'代码')}">
                        <td align="center"><img src="../../images/icon_chakan.gif" width="16" height="16" onclick="window.open('/bsweb/certificate_viewFddbrPage.action?jgdm=${tjgdmsave.jgdm}&sourceTable=${tjgdmsave.tableName}_bs')" style="cursor:pointer" title="查看"/></td>
                    </c:when>
                    <c:otherwise>
                    	
                        <td align="center"><img src="../../images/icon_chakan.gif" width="16" height="16" onclick="window.open('/bsweb/certificate_viewFddbrPage.action?jgdm=${tjgdmsave.tyshxydm}&sourceTable=${tjgdmsave.tableName}')" style="cursor:pointer" title="查看"/></td>
                    </c:otherwise>
                </c:choose>
      		</tr>
      	</c:forEach>
