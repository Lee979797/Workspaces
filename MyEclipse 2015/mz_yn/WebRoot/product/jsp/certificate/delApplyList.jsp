...
function checkCode(jgdm,type,delFlag){
    dwr.engine.setAsync(false);
    var codeflag = false;
    spBus.isAudiaAll(jgdm,'00', function (data) {
    ...
    if(!codeflag){
        if(type=='audit'){
            window.location.href="/bsweb/certificate_viewPage.action?jgdm="+jgdm+"&type=1&formType=4&delFlag="+delFlag+"&pageno=${pageno}&rowNumsView=${rowNumsView}";
        }else
            window.location.href="/bsweb/certificate_viewPage.action?jgdm="+jgdm+"&type=1&formType=4&pageno=${pageno}&rowNumsView=${rowNumsView}";
    }
}
</script>
</head>
      		...
      		<tr ${i.count % 2==1?"class='list_tr'":"class='list_tr2'" }>
                ...
                <td><fmt:formatDate value="${tjgdmsave.bzrq}" pattern="yyyy-MM-dd" /></td>
      			<c:if test="${tjgdmsave.flag=='0'}">
                    <td>已提交审核</td>
                    <td align="center"><img src="../../images/icon_chakan.gif" width="16" height="16" onclick="checkCode('${tjgdmsave.jgdm}','audit','0');" style="cursor:pointer" title="查看"/></td>
                </c:if>
                <c:if test="${tjgdmsave.flag=='1' and fn:trim(tjgdmsave.shflag)=='1'}">
                    <td>审核通过</td>
                    <td align="center"><img src="../../images/icon_del.gif" width="16" height="16" onclick="checkCode('${tjgdmsave.jgdm}','audit','1');" style="cursor:pointer" title="删除"/></td>
                </c:if>
                <c:if test="${tjgdmsave.flag=='1' and fn:trim(tjgdmsave.shflag)=='0'}">
                    <td>已审核,不允许删除</td>
                    <td align="center"><img src="../../images/report_go.png" width="16" height="16" onclick="checkCode('${tjgdmsave.jgdm}','audit','3');" style="cursor:pointer" title="重新提交审核"/></td>
                </c:if>
                <c:if test="${tjgdmsave.flag=='2'}">
                    <td>需要审核</td>
                    <td align="center"><img src="../../images/report_go.png" width="16" height="16" onclick="checkCode('${tjgdmsave.jgdm}','audit','2');" style="cursor:pointer" title="提交审核"/></td>
                </c:if>
                <c:if test="${!needAudit}">
                <td align="center"><img src="../../images/icon_del.gif" width="16" height="16" onclick="checkCode('${tjgdmsave.jgdm}','no','0');" style="cursor:pointer" title="删除"/></td>
                </c:if>
      		</tr>
      	</c:forEach>
    </table>
