...
<head>
    ...
    <script type="text/javascript">
        function fCheckValue() {
            ...
            }  else if(jgdm.value.substring(1,2)!='1'){
                ymPrompt.alert({message:"机构类型不正确！", width:330, height:220, handler:function () {
                    jgdm.focus();
                }});
            } else{
...
<div class="page_top">登记 >> 社会团体业务 >> 机构迁址</div>
<div id="list_main">
    <form method="POST" name='thisForm' action="/bsweb/change_searchBy.action">
        <%-- <input type="hidden" name="source" value="${source}" id="source"/>
        <input type="hidden" name="bzjgdm" value="${sysUser.bzjgdm}" id="bzjgdm"/>
		<input type="hidden" name="isdy" id="isdy" value="1"> --%>
        <div class="list_box">
