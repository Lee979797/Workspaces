   ...
   <td class=td1 align=right >
    ...
    <td class=td1>
     <c:set var="date" value="${jgdm.zfrq}"/>
        ...
        <c:if test="${!(date eq null)}">
            <fmt:formatDate value="${date}" pattern="yyyy-MM-dd"/>
        <input type="hidden" value=" <fmt:formatDate value="${date}" pattern="yyyy-MM-dd"/>" name="jgdm.zfrq">
        </c:if>
    </td>
</tr>
<TR>
...
<TR>
	...
	<td class="td1" align="left">
		${(jgdm.cishan eq '' or jgdm.cishan eq null or jgdm.cishan eq '2')?"否":"是" }
	</td>
	<td class="td1" align="right">是否取得募捐资格</td>
	<td class="td1" align="left">
		${(jgdm.mujuan eq '' or jgdm.mujuan eq null or jgdm.mujuan eq '2')?"否":"是" }
	</td>
</TR>
<c:if test="${'1' eq jglx}">
<TR>
	...
	<td class="td1" align="left">
		${(jgdm.tuogou eq '' or jgdm.tuogou eq null or jgdm.tuogou eq '2')?"否":"是" }
	</td>
...
<!-- lvwei  20170420  直接登记类型添加 start -->
<TR>
	<td class="td1" align="right">直接登记类型</td>
	<td class="td1" align="left">
		${(jgdm.zjdjlx eq '1')?"非直接登记类":"" }
		${(jgdm.zjdjlx eq '2')?"公益慈善类直接登记":"" }
		${(jgdm.zjdjlx eq '3')?"行业协会商会类直接登记":""}
		${(jgdm.zjdjlx eq '4')?"科技类直接登记":"" }
		${(jgdm.zjdjlx eq '5')?"社区服务类直接登记":"" }
		${(jgdm.zjdjlx eq '6')?"其他类登记类型":"" }
		${(jgdm.zjdjlx eq '' or jgdm.zjdjlx eq null)?"":""}
	</td>
	<td class="td1" align="right">是否为志愿组织</td>
	<td class="td1" align="left">
		${(jgdm.zhiyuan eq '' or jgdm.zhiyuan eq null or jgdm.zhiyuan eq '2')?"否":"是" }
	</td>
</TR>
<!-- lvwei  20170420  直接登记类型添加 end -->
	...
	<td class=td1 align=right>
     ...
     <TD class="td1" style="position:relative;display:block;overflow:visible;">
      <c:set var="name" value="${fn:trim(jgdm.mobile)}"/>
        ${(name eq '' or name eq null)?"":name }
        <input type="hidden" value="${jgdm.mobile }" name="jgdm.mobile">
    </TD>
    	<td class=td1 align=right>
            法定代表人座机：
    </td>
...
<TR>
    ...
    <td class=td1>
        <fmt:formatNumber value='${jgdm.zczj }' pattern="##.0000" minIntegerDigits="1"/>万元
    </td>
    <td class=td1 align=right>
        货币种类：
    </td>
