    ...
    <c:if test="${! empty models}">
        <script type="text/javascript">
            ...
            function makeGastaxDataVortex() {
                ...
                // Axes
                var axisType = new Axis("直接登记类型");
                var axisZone = new Axis("数量");
                ...
                zone = axisZone.makeNewBucket("数量");
                ...
                if (index == -1) {
                	if(${model.column1 eq '1'}){
                		tp = axisType.makeNewBucket("非直接登记类");
                	}else if(${model.column1 eq '2'}){
                		tp = axisType.makeNewBucket("公益慈善类直接登记");
                	}else if(${model.column1 eq '3'}){
                		tp = axisType.makeNewBucket("行业协会商会类直接登记");
                	}else if(${model.column1 eq '4'}){
                		tp = axisType.makeNewBucket("科技类直接登记");
                	}else if(${model.column1 eq '5'}){
                		tp = axisType.makeNewBucket("社区服务类直接登记");
                	}else if(${model.column1 eq '6'}){
                		tp = axisType.makeNewBucket("其他类直接登记");
                	}else if(${model.column1 eq 'sum'}){
                		tp = axisType.makeNewBucket("总计");
                	}
                    type_values.push(tp);
...
<div id="list_main">
    <form name="searchForm" method="post" action="/bsweb/statistics_rateZjdjlxBus.action">
        ...
        <div class="list_box_top" style="border: 1px;">
            时间：
            <INPUT   maxLength=10 size=23
                   name="startDate" id="startDate" onclick="WdatePicker({el:$dp.$('startDate')});"
            ...
            </A>至<INPUT   maxLength=10
                        size=23 onclick="WdatePicker({el:$dp.$('endDate')});"
        ...
        <INPUT class="button" onClick="return selectUpWindow('xzqh');" type="button" value=选择
               name="btselect2"/>&nbsp;
               <span
            style="position:absolute; top:23px; left:700px;"
