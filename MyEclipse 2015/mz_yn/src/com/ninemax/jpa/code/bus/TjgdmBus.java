...
public class TjgdmBus {
       ...
       /*     if ((tJgdm.getDybz().equals("0")) && (!tJgdm.getJglx().equals("B")) && (!tJgdm.getJglx().equals("b"))) {
          ...
          /*  List<TSmrw> smrws = em.createQuery("select model from TSmrw model where  model.jgdm=?1 and (model.type in (0,3,4,6,9))")
            ...
            if("1".equals(jglx)){
            	...
            	czjl.setMemo("社团申请表修改!");
            }else if("2".equals(jglx)){
            	czjl.setMemo("民非申请表修改!");
            }else if("3".equals(jglx)){
            	czjl.setMemo("基金会申请表修改!");
            }
            em.persist(czjl);
            em.flush();
            result = 1;
    ...
    public List<TjgdmVO> listTjgdm(User user, Map<String, String> params, Integer pageno, Integer rowNumsView, clsPageComponent pages, String orderbyColum, String orderbyMethod,String jglx,String tyshxydm) {
        ...
        try {
            ...
            //以下注释的是原版的信息修改，只查询dybz=0的数据，因为导入功能导入的是dybz=2的数据，所以现在改成dybz>=0
//            String jql = "select model.tyshxydm,model.jgmc,model.bzrq,s.flag,s.shflag,model.xzqh,model.zch from t_jgdm model left join (select tyshxydm,flag,shflag from  t_sp  where  flag !=2 and ywlx in ('16') ) s on model.tyshxydm = s.tyshxydm where " + cond + " and model.dybz = '0' and model.jglx="+jglx;
            String jql = "select model.tyshxydm,model.jgmc,model.bzrq,s.flag,s.shflag,model.xzqh,model.zch from t_jgdm model left join (select tyshxydm,flag,shflag from  t_sp  where  flag !=2 and ywlx in ('16') ) s on model.tyshxydm = s.tyshxydm where " + cond + " and model.dybz >= '0' and model.jglx="+jglx;
   ...
   /*         String jql = "select new com.ninemax.jpa.code.model.vo.TjgdmVO(model.jgdm,model.jgmc,model.bzrq,model.tyshxydm,model.bzjgdm,model.zch) "
            		+ " from TJgdm model "
            		+ " where "+cond+" and model.jglx='"+jglx+"' and "
            		+ "  model.dybz='1' and  "
            		+ " (model.tyshxydm like '" + "%" + ("".equals(tyshxydm) ? "" : tyshxydm) + "%" + "' "
            		+ " or model.jgdm like '" + "%" + ("".equals(tyshxydm) ? "" : tyshxydm) + "%" + "')";*/
            String jql = "select new com.ninemax.jpa.code.model.vo.TjgdmVO(model.jgdm,model.jgmc,model.bzrq,model.tyshxydm,model.bzjgdm,model.zch) "
            		+ " from TJgdm model "
            		+ " where "+cond+" and model.jglx='"+jglx+"' and "
            		+ "  model.dybz='1' and  "
            		+ " (model.tyshxydm like '" + "%" + ("".equals(tyshxydm) ? "" : tyshxydm) + "%" + "' "
            		+ " or model.jgdm like '" + "%" + ("".equals(tyshxydm) ? "" : tyshxydm) + "%" + "' "
            		+ " or model.jgmc like'"+"%"+("".equals(tyshxydm)?"":tyshxydm)+"%"+"')";
            List<Object> pms = new ArrayList();
            if (params != null && params.size() > 0) {
                for (Map.Entry<String, String> param : params.entrySet()) {
                    String key = param.getKey();
    ...
    public int AddTjgdm(TJgdm jgdm) {
   	 
    	if (tJgdmDAO.save(jgdm)) {
    ...
    public int deleteTjgdm(TJgdm jgdm){
    	if(tJgdmDAO.delete(jgdm)){
    		return 1;
    	}else{
    		return 0;
    	}   	
    ...
    public List findAllTyshxydm(String sql) {
      	List list = tJgdmDAO.findbysql(sql);
    	return list;
    }
    
    public List findAllJgmc(String sql) {
      	List list = tJgdmDAO.findbysql(sql);
    	return list;
    }
