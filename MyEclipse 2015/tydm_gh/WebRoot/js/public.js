...
function checkItem1(obj,title) {
    if ((obj && obj.value && obj.value.trim().indexOf("工会")==-1)) {
        ymPrompt.alert(title + "中必须包含有‘工会’两字!!", 330, 220, "提示信息", function (data) {
            if (data == "ok") {
                obj.focus();
            }
        });        
    ...
    if ((obj && obj.value && obj.value.trim().indexOf("工会办事处")!=-1)) {
        ymPrompt.alert(title + "‘工会办事处’不属于工会法人!!", 330, 220, "提示信息", function (data) {
            if (data == "ok") {
                obj.focus();
            }
        });
    ...
    if(obj && obj.value && obj.value.trim().substring(obj && obj.value && obj.value.trim().length-5,obj && obj.value && obj.value.trim().length-0) == '工作委员会'){
        ymPrompt.alert(title + "‘工作委员会’不属于工会法人!!", 330, 220, "提示信息", function (data) {
            if (data == "ok") {
                obj.focus();
            }
        });
...
//若不为空,判断各个项目的内容
function fCheckEssItem(type,frmThis) {
    var allValid = true;
    var strName;
