					...
					<div id="cs">
						...
						<div class="Contentbox1">
								...
								if (rightKeys != null && rightKeys.size() > 0) {
									...
									for (Rightkey _rightKeyTO : rightKeys) {
										...
										if (userRightKeyBus.HasRight(
												String.valueOf(user.getUserId()), _rightKeyId)) { //如果有权限才显示
							%>
							...
							out.print("style='display:none'");
								...
								<%
									List<Rightkey> twoRightKeys = rightKeyBus
														.ListFirstRight(_rightKeyId);
												if (twoRightKeys != null && twoRightKeys.size() > 0) {
													...
													for (Rightkey rightKeyTO : twoRightKeys) {
														...
														if (userRightKeyBus.HasRight(
																...
																rightKeyId)) { //如果有权限才显示
															List<Rightkey> child_rightKeys = rightKeyBus
																	.ListFirstRight(rightKeyId);//子类型
															if (child_rightKeys != null
																	&& child_rightKeys.size() > 0) {
																//有子节点
								%>
								<div class="titl_bg" id="div<%=_rightKeyId%><%=userNum%>"
									onclick="showTabPage('divsub<%=_rightKeyId%>','divsub<%=_rightKeyId%><%=userNum%>','<%=String.valueOf(user
												.getUserId())%>','<%=_rightKeyId%>')"
											...
											<%
												//循环子节点
																			for (Rightkey child_rightKeyTO : child_rightKeys) {
																				String child_rightKeyId = child_rightKeyTO
																						.getRightkeyId();
																				String child_linkPage = child_rightKeyTO
																						.getLinkpage();
																				String child_title = child_rightKeyTO
																						.getRightkeyName();
																				if (clsStringTool
																						.isEmpty(child_linkPage)) {
																					child_linkPage = "";
																				}
																				if (userRightKeyBus.HasRight(String
																						.valueOf(user.getUserId()),
																						child_rightKeyId)) { //如果有权限才显示
											%>
											<li><a
												href="../jsp/comm.jsp?q=<%=LinkEncrypt
														.encrypt(child_linkPage)%>"
												target="kunFrame"><%=child_title%> </a></li>
								...
								<div class="titl_bg" id="div<%=_rightKeyId%><%=userNum%>"
									target="kunFrame">
									<a href="../jsp/comm.jsp?q=<%=LinkEncrypt.encrypt(linkPage)%>"
										target="kunFrame"><span><%=title%></span></a>
								</div>
