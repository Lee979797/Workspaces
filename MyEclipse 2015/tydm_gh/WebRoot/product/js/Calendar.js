...
function createyear(year,obj)//创建年份选择
    ...
    var oDiv;
      ...
      ystr+="<tr><td style='cursor:pointer' onclick='createyear("+(year-20)+",Cal_Top.cells[2])' align=center>上翻</td></tr>";
      for(i=year-10;i<year+10;i++)
        if(year==i)
        ystr+="<tr style='background-color:#909eff'><td style='color:black;height:11px;cursor:pointer' align=center onclick='Cal_Top.cells[2].innerText=this.innerText;showDate("+i+",parseInt(Cal_Top.cells[3].innerText));Cal_Day.parentElement.nextSibling.innerHTML=\"\"'>"+i+"年</td></tr>";
        else
        ystr+="<tr><td align=center style='cursor:pointer' onclick='Cal_Top.cells[2].innerText=this.innerText;showDate("+i+",parseInt(Cal_Top.cells[3].innerText));Cal_Day.parentElement.nextSibling.innerHTML=\"\"'>"+i+"年</td></tr>";
      ystr+="<tr><td style='cursor:pointer' onclick='createyear("+(year+20)+",Cal_Top.cells[2])' align=center>下翻</td></tr>";
      ystr+="</table>";
...
function createmonth(month,obj)//创建月份选择
{
    ...
    var oDiv;
      ...
      for(i=1;i<13;i++)
        if (month==i)
        mstr+="<tr style='background-color:#909eff '><td style='color:black;height:11px;cursor:pointer' align=center onclick='Cal_Top.cells[3].innerText=this.innerText;showDate(parseInt(Cal_Top.cells[2].innerText),"+i+");Cal_Day.parentElement.nextSibling.innerHTML=\"\"'>"+i+"月</td></tr>";
        else
        mstr+="<tr><td align=center style='cursor:pointer' onclick='Cal_Top.cells[3].innerText=this.innerText;showDate(parseInt(Cal_Top.cells[2].innerText),"+i+");Cal_Day.parentElement.nextSibling.innerHTML=\"\"'>"+i+"月</td></tr>";
      mstr+="</table>";
...
function getCalendar()
{
  ...
  for(i=0;i<7;i++)
	...
	str=str+"</tr></table>";
	//创建日期条目
	str=str+"<table id='Cal_Day' cellpadding=1 cellspacing=1  bgcolor=#79A7E2  style='width:203px;font-size:11px;cursor:pointer;height:8px;'>";
	for(i=0;i<6;i++)
	{
		str=str+"<tr>";
		for(j=0;j<7;j++)
	...
	str=str+"<span title='减少月份' onmouseover='this.style.color=\"black\"' onclick='cutMonth()' onmouseout='this.style.color=\"\"' width=10 style='font-family: Webdings;cursor:pointer;color:#7F8E91;'>3</span>";
	...
	str=str+"<span width=10 onmouseover='this.style.color=\"black\"' onmouseout='this.style.color=\"\"' onclick='addMonth()' style='font-family: Webdings;cursor:pointer;color:#7F8E91;' title='增加月份'>4</span>";
	str=str+"<br>";
	str=str+"<span width=10 onmouseover='this.style.color=\"black\"' onmouseout='this.style.color=\"\"' onclick='cutYear()' style='font-family: Webdings;cursor:pointer;color:#7F8E91;' title='减少年份'>7</span>";
	str=str+"<span style='font-size:11px;font-weight:bold; color:#7F8E91;FONT-FAMILY: Verdana, Arial, Helvetica, sans-serif'>YEAR</span>";
	str=str+"<span width=10 style='font-family: Webdings;cursor:pointer; color:#7F8E91;' onmouseover='this.style.color=\"black\"' onmouseout='this.style.color=\"\"' onclick='addYear()' title='增加年份'>8</span>";
	str=str+"</td>";
	str=str+"</tr></table>";  
	return str;
}