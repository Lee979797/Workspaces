    ...
    <script type="text/javascript">
        ...
        function checkCode(jgdm, type, id, sourceTable) {
                ...
                if (type == 'update') {
                    ...
                    window.location.href = "/bsweb/certificate_updatePageSave.action?tyshxydm=" + jgdm + "&id=" + id + "&formType=${formType}&pageno=${pageno}&rowNumsView=${rowNumsView}&nav=1&sourceTable=" + sourceTable;
                } else{
                    window.location.href = "/bsweb/certificate_viewPage.action?djh=${djh}&id=" + id + "&type=2&formType=${formType}&pageno=${pageno}&rowNumsView=${rowNumsView}&nav=1";
                }
...
<body>
<div class="page_top">登记  &gt;&gt; 工会业务 &gt;&gt; <%if ("1".equals(formType)) {%>信息补录<%}else{ %> 设立登记<%} %>
</div>
...
<div class="list_box_top">
    ...
    <input name="button2" type="button"  class="newBtn1" value="新 增"
          onclick="window.location.href='/product/jsp/certificate/addinfomationEnter.jsp?formType=<%=formType%>'"/>
           <%if ("0".equals(formType)) {%>
           <input name="button2" type="button"  class="newBtn1" value="查询新增"
          onclick="window.location.href='/product/jsp/certificate/search_gh.jsp'"/>
          <%}%>
</div>
