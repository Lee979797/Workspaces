...
<head>
    ...
    <script type="text/javascript">
        function fm() {
            ...
            tjgdmSaveBus.canFm(<%=tJgdm.getId()%>, function (flag) {
             if (flag) {
                   
                        window.location.href = '/bsweb/certificate_revision2.action?djh=${djh}&ywlsh=${ywlsh}&id=<%=tJgdm.getId()%>&formType=<%=formType%>&type=<%=type%>&path=${path}&jgdm='+jgdm;
        ...
        function del() {
            if (confirm("确认要删除？")) {
                window.location.href = '/bsweb/certificate_delete.action?id=<%=tJgdm.getId()==null?0:tJgdm.getId()%>&jgmc=<%=clsStringTool.convertNull(tJgdm.getJgmc())%>&formType=<%=formType%>&type=<%=type%>&pageno=${pageno}&rowNumsView=${rowNumsView}';
            }
        }
        function send() {
...
<div class="page_top_fixed">
    <div align="left" style=" float: left;"><strong> 发证 &gt;&gt; <%=tit%> &gt;&gt; <%=title%>
    </strong>
    </div>
    ...
    %>
        ...
        <%}%>
           ...
           <%
                        ...
                        if("6".equals(type)){
                        	%>
                        	 <INPUT class="newBtn1" onClick="window.location.href='/bsweb/certificate_pass?id=<%=tJgdm.getId()==null?0:tJgdm.getId()%>&ywlsh=1'"
                           type=button value="通过" name="btok"/>
                            <INPUT class="newBtn1" onClick="window.location.href='/bsweb/certificate_pass?id=<%=tJgdm.getId()==null?0:tJgdm.getId()%>&ywlsh=2'"
                           type=button value="不通过" name="btok"/>
                        	<% 
                        }
                    %>
                    ...
                    <TABLE class=tableBorder0 cellSpacing=0 cellPadding=5 align=center border=0 width="100%"
                        ...
                        %>
						   <TR>
                            ...
                            <TD class=td1 colspan="3">
                               <input type="text" name="jgdm" value="" maxlength="9"/>
                            </TD>
                        </TR>
