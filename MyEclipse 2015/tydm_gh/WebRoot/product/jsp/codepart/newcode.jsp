...
<head>
    ...
    <script type="text/javascript">
        ...
        function fCheckValue() {
            ...
            if (qsmd.length != 8 || jzmd.length != 8 ) {
                ymPrompt.alert({message: '您输入的值不足8位！', width: 330, height: 220, title: '提示信息'});
                return false;
            }
            ...
            if (!(reg.test(qsmd) && reg.test(jzmd) )) {
                //ymPrompt.alert({message: '请输入正确的码段：以连续3位字母开头！', width: 330, height: 220, title: '提示信息'});
               //// return false;
            ...
            codePart.getMdsl(qsmd, jzmd, function (num) {
                if (num > 20000) {
                	...
                	ymPrompt.alert({message: '您输入的起始码段大于截止码段！', width: 330, height: 220, title: '提示信息', handler: function () {
                        document.getElementById("jzmd").focus();
                    }});
                }else {
                    nub = num;
...
<form name="frmThis" method="post" action="/bsweb/codenewcode.action">
