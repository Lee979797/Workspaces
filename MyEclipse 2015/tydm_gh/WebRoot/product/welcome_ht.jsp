...
.welBan {
	...
	height: 377px;
	margin: 0px auto;
	background: url(images/welcome_ht.jpg) no-repeat;
	overflow: hidden;
}
...
<link rel="stylesheet" type="text/css"
	href="/js/highslide-4.1.8/highslide/highslidezx.css" />
<script type="text/javascript"
	src="/js/highslide-4.1.8/highslide/highslide-with-html.js"
	charset="gbk"></script>
<script type="text/javascript">
...
<script type="text/javascript">
	function showBoard(sysId, title, content) {
		document.getElementById("show").innerHTML = "<h3>" + title + "</h3>"
				+ content;
	...
	function addUpload(fileName, fileId, saveName) {
		...
		var newDiv = "<div id=divUpload" + fileId + ">"
				+ "<img src='/product/jsp/images/fujian_tubiao.jpg' /><a href='" + url + saveName + "' >"
				+ fileName + "</A>" + "</div>";
	...
	function aa() {
		...
		ymPrompt.win('/product/jsp/user/modPassword.jsp', 400, 300, '密码修改',
				null, null, null, {
					id : 'a'
				})
	}
...
<body
	...
	<form name="searchForm"
		action="<%=request.getContextPath()%>/servlet/DownServlet"
		method="get">
		<input type="hidden" id="path" name="path" value="<%=savePath%>" />
	</form>
	<span style="margin-left:50%;" id="btn"
		onclick="return hs.htmlExpand(this, { headingText: '系统公告' })">&nbsp;</span>
	...
	<div class="welcome_ht">
		...
		<div class="imgGg">
			<img src="../images/icon_volume.gif" /> <b
			...
			<div id="holder">
				<ol>
					<c:forEach varStatus="i" var="gonggao" items="${messageList}">
						<li
							onclick="showBoard('${gonggao.sys_id}','${gonggao.send_title}','${fn:escapeXml(gonggao.send_content)}');">${gonggao.send_title}
			...
			<tr>
				<td width="50%" style="vertical-align:top;">
					<div class="home_footer_box">
						<h1>用户信息</h1>
						<ul>
							<li>用户名:${sysUser.userName } [<a href="#"
								onclick="javascript:aa()" title="点击修改密码">修改密码</a>]
							</li>
				...
				<td width="50%" style="vertical-align:top;">
					<div class="home_footer_box">
						...
						<ul>
							...
							<li>操作系统版本:<%=useros%></li>
							<li>浏览器类型:<%=userbrowser%>
							</li>
							<li><a
								style="text-decoration:underline;color:blue;font-weight:bold;"
								href="/icocx/install_lodop.exe">打印控件下载</a></li>
						</ul>
					</div>
				</td>
			</tr>
