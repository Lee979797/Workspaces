    ...
    public String inDataList() {
        return new ActionUtils() {
            ...
            protected void excute() throws Exception {
                ...
                String sql = " from TSp model where model.flag='0'";
    ...
    public String _findInSameName() {
        return new ActionUtils() {
            ...
            public void get(int type, String... mcs) {
                ...
                if(type == 5){
                sql = "SELECT jgdm,jgmc,fddbr,jglx,'BS注销库' AS sjly,'t_fzdm_bs' AS tablename,zch,0,fzrq FROM t_fzdm  where jgdm = '" + jgdm.getTyshxydm().substring(8, 17).trim() + "' ";
            	...
            	returnList = querybs.getResultList();
                 ...
                 sql = "SELECT jgdm,jgmc,fddbr,jglx,'BS注销库' AS sjly,'t_fzdm_bs' AS tablename,zch,0,fzrq FROM t_fzdm  " + where + " and jgdm <> '" + jgdm.getTyshxydm().substring(8, 17).trim() + "' ";
    ...
    public String auditIndata() {
        return new ActionUtils() {
            ...
            protected void excute() throws Exception {
                ...
                sp1.setFlag("1");
                em.merge(sp1);
                TFzdm fzdm = em.find(TFzdm.class, sp1.getTyshxydm());
                TJgdm dm = new TJgdm();
                BeanUtilsEx.copyProperties(dm,fzdm );
                em.persist(dm);
                String sql = "delete TFzdm where jgdm='" + fzdm.getJgdm() + "'";
                em.createQuery(sql).executeUpdate();
                resultMsg =  "注销恢复:"+"(流水号：'" + sp1.getLsh() + "')，审核完毕";
                currentPath = path + "success.jsp";
                source = "inDataList";
