...
var grid_pzjg_ulmo = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	...
	listeners : {
		'rowclick':function(grid,rowIndex){
			window_pzjgQuery_ulmo.hide();
			var selections=grid.getSelectionModel().getSelections(); 
			orgnewMoveoutForm.getForm().findField('select_pzjgmc_ulmo').setValue(selections[0].get('pzjgmc'));
			orgnewMoveoutForm.getForm().findField('select_pzjgdm_ulmo').setValue(selections[0].get('pzjgdm'));
		}
	}
});


var window_pzjgQuery_ulmo = new Ext.Window({
	title : '登记批准机构查询',
	iconCls : 'icon-plugin',
	width :500,
	height:400,
	...
	maximizable:true,
	closeAction : 'hide',
	items : [grid_pzjg_ulmo]
});
	         
var orgnewMoveoutForm = new Ext.FormPanel({
	loadMask : {
		msg : '数据加载中...'
	},
	...
	items : [{//第一行修改
		...
		items : [{
			...
			items : [{
				...
				defaultType : 'textfield',
				defaults : {anchor : '98%'},
				items : [{fieldLabel : '法定代表人(负责人)名称',name :'fddbr',allowBlank : false,readOnly:true,maxLength : 20,defaults : {anchor:'98%'}}]
			},{
				columnWidth : .34,
				layout : 'form',
				border : false,
				...
				defaultType : 'textfield',
				defaults : {anchor : '98%'},
				items :[{fieldLabel : '法定代表人(负责人)证件号码',name : 'zjhm',allowBlank : false,readOnly:true,maxLength : 18,defaults : {anchor:'95%'}}]
			}]
		}]
	},{//第三行修改
		layout : 'column',
		...
		items : [{
				...
				items : [{
					...
					items : [{fieldLabel : '经营或业务范围',name : 'jyfw',allowBlank : false,readOnly:true,maxLength : 240,defaults : {anchor:'100%'}}]
				}]
		}]
	},{//第四行修改
		layout : 'column',
		...
		items : [{
			...
			xtype : 'hidden',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '职工人数',name : 'zgrs',readOnly:true,xtype:'numberfield',maxLength:2000000000}]
		},{
			columnWidth : .05,
			layout : 'form',
			border : false,
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '企业登记注册类型',	name : 'jjlx',allowBlank : false,readOnly:true,maxLength : 50,defaults : {anchor:'98%'}}]
		},{
			columnWidth : .5,
			layout : 'form',
			baseCls : 'x-plain',
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '外商投资国别或地区',name : 'wftzgb',readOnly:true,maxLength : 50}]
		}]
	},{//第六行修改
		layout : 'column',
		border : false,
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '注册(开办)资金',xtype : 'numberfield',name : 'zczj',readOnly:true,maxValue : 2000000000}]
		},{
			columnWidth : .05,
			layout : 'form',
			border : false,
			...
			items :[{
				...
				listeners : {
					'select' : function(combo, record, index) {
						//orgnewAuditForm.findField('hbzl').setValue(record.data.categoryName);
					}
				}
		    }]
		}]
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items :[{fieldLabel : '主管部门名称',name : 'zgmc',readOnly:true,maxLength : 80}]
		},{
			columnWidth : .5,
			layout : 'form',
			baseCls : 'x-plain',
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '主管部门代码',name : 'zgdm',readOnly:true,maxLength : 9}]
		}]
	},{//第六行修改
		layout : 'column',
		border : false,
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{id:"select_pzjgdm_ulmo",fieldLabel : '登记批准机构代码',name : 'pzjgdm',readOnly:true,maxLength : 9}]
		}]
	},{//第六行修改
		layout : 'column',
		border : false,
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items :[{fieldLabel : '批准文号(注册号)',name : 'zch',readOnly:true,maxLength : 31}]
		},{
			columnWidth : .5,
			layout : 'form',
			baseCls : 'x-plain',
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items : [{fieldLabel : '邮政编码',name : 'yzbm',readOnly:true,maxLength : 6}]
		}]
	},{	//第六行修改
		layout : 'column',
		border : false,
		...
		items : [{
			...
			defaultType : 'textfield',
			defaults : {anchor : '99%'},
			items :[{fieldLabel : '机构地址',name : 'jgdz',allowBlank : false,readOnly:true,maxLength : 100,defaults : {anchor:'99%'}}]
		...
		items : [{
			...
			items :[{	
					...
					listeners : {
						'select' : function(combo, record, index) {
							orgnewMoveoutForm.getForm().findField('_xzqhName').setValue(Ext.get("xzqh_ulmo").dom.value);
						}
					}
				}]
		},{
			...
			defaultType : 'textfield',
			defaults : {anchor : '98%'},
			items :[{xtype : 'hidden',fieldLabel : '固定电话号码',name : 'dhhm',readOnly:true,maxLength : 21}]
		}]
	},{ //表单修改
		layout : 'column',
		border : false,
		...
		items : [{
			...
			defaults : {anchor : '98%'},
			items : [
			    {xtype : 'hidden',fieldLabel : '移动电话',name : 'mobile',readOnly:true,maxLength : 25},
			    {xtype : 'hidden',fieldLabel : '机构网址',name : 'weburl',readOnly:true,allowBlank : false,maxLength : 50},
			    ...
			    {fieldLabel : '迁入地址',name : 'moveoutAddrss',allowBlank : false,maxLength : 100},
			...
			defaults : {anchor : '98%'},
			items : [
			    {xtype : 'hidden',fieldLabel : '电子邮箱',name : 'email',readOnly:true,maxLength : 40},
			    {fieldLabel : '经办人电话',name : 'tbrlxfs',allowBlank : false,maxLength : 25},
			    {fieldLabel : '经办人证件号码',name : 'tbrsfzh',allowBlank : false,maxLength : 18},
			    {fieldLabel : '迁址原因',name : 'moveoutReason',allowBlank : false,readOnly : false,maxLength : 250},
				...
				{xtype : 'hidden',name : 'newYwlx',value:'迁出'},
				{xtype : 'hidden',name : 'newState',value:60}
			]
		}]
	}],	
	buttonAlign : 'center',
	...
	buttons : [{
		...
		handler : function(btn){
			if (orgnewMoveoutForm.getForm().isValid()) {
				btn.disable();
				orgnewMoveoutForm.getForm().submit({
					waitTitle : '请稍候',
					waitMsg : '正在保存信息,请稍候...',
					success : function(form,action) {
						Ext.Msg.show({
							title : '提示',
							msg : '“迁出申请单”保存成功!<br><br>下一步，扫描上传相关审批文件',
							buttons : Ext.Msg.OK,
							fn : function(){
								btn.enable();
								var tab7=Ext.getCmp("centerPanel7");
								tab7.setActiveTab("MoveoutScTab");
								
								var tab31=Ext.getCmp("MoveoutTab");
								tab31.setTitle("第一步、迁出信息录入(已保存)");	
		...
		hidden:false,
		handler :function() {
			orgnewMoveoutForm.getForm().reader = Orgnews;
			orgnewMoveoutForm.getForm().load({
			    url: 'findUsernameOrgnew.action?username='+currentUsername+'&stateConditions=100,60,63', //请求控制器获取数据
			    //url: 'findUsernameOrgnew2.json',
			    method: 'POST', //为"POST",则上面的url不会显示在浏览器的地址栏中
			    waitTitle: '提示',
			    waitMsg: '数据正在加载中，请稍后',
			    success:function(form,action){
			    	var resultObject861 = Ext.util.JSON.decode(action.responseText);
   					//alert("pageTypeStr="+resultObject86.root[0].pageTypeStr);
		    		...
		    		//alert(resultObject861.root[0].zcrq);
		    		//alert(action.responseText);
			    	//orgnewMoveoutForm.getForm().findField('zcrq').setValue(resultObject861.root[0].zcrq.format('Y-m-d'));
			    }
			});
...
var MoveoutScOrgnewForm = new Ext.Panel({
	//url : 'shangchuanOrgnew.action',
	//bodyStyle : 'padding:5px',
	border : false,
	items: [imgForm],
	html: '<OBJECT classid="clsid:B5306E2E-8F98-46CA-9F31-AB326B2613F1"  width=100%  height=100% align=center hspace=0 vspace=0 id="scanner86"  name="scanner86" >'
		...
		buttons: [{
		   ...
		   text: '上传',   
		       handler: function(btn) {   
						...
						//scanner86.ImageData=document.getElementById("ImageData").value;
						base64file = scanner86.ImageData;  //获取控件扫描的图片数据
						var baseSize = base64file.length;  //获取控件图片数据的64位编码的长度
						imageCount = scanner86.GetPageCount;	//获取扫描图片的页数
						pageTypeStr = scanner86.PageType;    //获取标识字符串,需要写数据库
						if(pageTypeStr!=""){
						...
						if(scanner86.CheckOrder()!="")   //判断如果标识数据的类别是否完整
						{
							alert(scanner86.CheckOrder());
							btn.enable();
							return false;
						}
						scanner86.OpenProgress(packCount);  //设置上传的进度条的总进度数
						...
						for(i=0; i < packCount; i++)  //分包上传
						{
							...
							try
							{
								...
								if(objs[0].success)
								{
									//scanner86.SetSendProgress(Math.ceil(i / packCount * 100));
									scanner86.Progress('上传中',i+1);
								}
								else
								{
									alert("上传失败，请重试0001。");
									//scanner86.ShowSendMsg(false);
									btn.enable();
									return false;
								}
							}
						...
						scanner86.CloseProgress();
						
						
						Ext.Msg.show({
							title : '提示',
							msg : '“扫描上传”成功！<br><br>&nbsp;&nbsp;下一步，进入“提交迁出申请”!',
							buttons : Ext.Msg.OK,
							icon : Ext.Msg.INFO,
							fn : function(){
								btn.enable();
								var tab3=Ext.getCmp("centerPanel7");
								tab3.setActiveTab("MoveoutTjTab");
								return true;
							}
						});
		       }   
...
//------------迁出信息中，查询返回结果的数据集----------
var ds_orgnewMoveout = new Ext.data.Store({
...
//----------------------点击列表信息，显示图书的扩展属性----------------------
var expander_imo = new Ext.grid.RowExpander({
	tpl : new Ext.Template('<table width="100%"  border="0" cellspacing="6" cellpadding="0"  bgcolor="#DDEEF9" style="margin-top:1px;" >'
			...
			+ '</tr>'
			+ '<tr height="22" valign="middle">'
			+ '  <td align="right" nowrap="nowrap" bgcolor="#efefef" style="padding-top:5px">迁入地址：</td><td nowrap="nowrap" style="padding-top:5px">{moveoutAddrss}<img src="images/temp.gif"></td>'
			+ '  <td align="right" nowrap="nowrap" bgcolor="#efefef" style="padding-top:5px">迁址原因：</td><td nowrap="nowrap" style="padding-top:5px">{moveoutReason}<img src="images/temp.gif"></td>'
			+ '</tr>'
			+ '<tr height="22" valign="middle">'
...
var btn_return_orgnewMoveout = new Ext.Button({
	text : '提交迁出',
	iconCls : 'icon-save',
	disabled:true,
	handler : function(btn){
		var record = grid_LicenseMoveout.getSelectionModel().getSelected();
		if(record){
   				...
   				success: function(){
   					Ext.Msg.show({
						...
						fn : function(){
							...
							//grid_LicenseMoveout.fireEvent('rowclick',grid_LicenseMoveout,grid_LicenseMoveout.getStore().indexOf(record));
							//grid_LicenseMoveout.getStore().remove(record);
						}
	   					
					});
   					var tab3=Ext.getCmp("centerPanel7");
   					tab3.remove("MoveoutTab");   //移除标签
					tab3.remove("MoveoutScTab");   //移除标签
					tab3.remove("MoveoutPtTab");   //移除标签
   				},
   				failure: function(){
   					Ext.Msg.show({
						title : '提示',
...
var btn_search_orgnewMoveout = new Ext.Button({
	text : '查询',
	hidden:true,
	iconCls : 'icon-search',
	handler : function() {
		searchOrgnewMoveout();
		//btn_add_orgnewMoveout.setDisabled(false);
		//btn_edit_orgnewMoveout.setDisabled(true);
		//btn_return_orgnewMoveout.setDisabled(true);
	}
});

var btn_refresh_orgnewMoveout = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchOrgnewMoveout.value='';
		searchOrgnewMoveout();
		//btn_edit_orgnewMoveout.setDisabled(true);
		btn_return_orgnewMoveout.setDisabled(true);
	}
...
var text_search_orgnewMoveout = new Ext.form.TextField({
	id : 'textSearchOrgnewMoveout',
	hidden:true,
	name : 'textSearchOrgnewMoveout',
	width : 200,
...
//--------------------- 默认查询 limit为显示的条数 ---------------------- 
var searchOrgnewMoveout = function() {
	ds_orgnewMoveout.baseParams.conditions = text_search_orgnewMoveout.getValue();
	ds_orgnewMoveout.baseParams.username=currentUsername;
	ds_orgnewMoveout.baseParams.stateConditions='60,61,62,63,64';
	ds_orgnewMoveout.load({params : {start : 0,limit : 10} });
};
...
var cm_orgnew = new Ext.grid.ColumnModel([expander_imo,
	sm32,
...
//---------------------- 加载右栏主页面   （数据列表、工具栏按钮）---------------------- 
var grid_LicenseMoveout = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	enableColumnMove : false,
	//autoHeight : true,
	...
	viewConfig : {forceFit : true},
	plugins : expander_imo,
	tbar : [btn_refresh_orgnewMoveout,'-',btn_return_orgnewMoveout,'->',text_search_orgnewMoveout,
	        btn_search_orgnewMoveout],
	bbar : new Ext.PagingToolbar({
		pageSize : 10,
		store : ds_orgnewMoveout,
		displayInfo : true,
		displayMsg : '第 {0} - {1} 条 共 {2} 条',
		emptyMsg : "没有工单记录"
	}),
	listeners : {
		'rowclick':function(grid,rowIndex){
			if(grid.getStore().getAt(rowIndex).data.state == 62){
				//btn_edit_orgnewMoveout.setDisabled(true);
				btn_return_orgnewMoveout.setDisabled(true);
			}else{
				if(grid.getStore().getAt(rowIndex).data.state == 61){
					//btn_edit_orgnewMoveout.setDisabled(true);
					btn_return_orgnewMoveout.setDisabled(true);
				}else{
					//btn_edit_orgnewMoveout.setDisabled(false);
					btn_return_orgnewMoveout.setDisabled(false);
				}
			}
		}
	}
...
//--------申请表打印中，查询返回结果的数据集---------
var ds_orgnewMoveout2 = new Ext.data.Store({
	url : 'findAllOrgnew.action',
	reader : new Ext.data.JsonReader(
		{totalProperty : 'totalProperty',root : 'root'}, 
	   [{name : 'orgid',type : 'int'},
...
//--------“申请表打印”中，列表显示机构的扩展属性------------
var expander_imo2 = new Ext.grid.RowExpander({
	tpl : new Ext.Template('<table width="100%"  border="0" cellspacing="6" cellpadding="0"  bgcolor="#DDEEF9" style="margin-top:1px;" >'
			+ '<tr>'
			+ '<td width="" colspan="4">'
			+ '<table width="98%" bgcolor="#ffffff" border="1" cellpadding="0" cellspacing="0" align="center"  bordercolordark="#FFFFFF" bordercolorlight="#6EBBE1" style="margin-top:0px;>'
...
//---------------------- 打印申请登记表 ---------------------------
function msgShow_ulmo()
{
	Ext.Msg.confirm('操作提示','“迁出申请表”打印完毕吗?',function(btn){
		if('yes' == btn){
			var tab3=Ext.getCmp("centerPanel7");
			tab3.setActiveTab("MoveoutScTab");
		}
		btn_print_orgnewMoveout2.setDisabled(false);	
	});	
}


var btn_print_orgnewMoveout2 = new Ext.Button({
	text : '打印申请表',
	iconCls : 'icon-print',
	disabled:false,
	handler : function(btn){
		//Ext.MessageBox.alert("提示","test"); 
		var record2 = grid_LicenseMoveout2.getSelectionModel().getSelected();
...
var btn_search_orgnewMoveout2 = new Ext.Button({
	text : '查询',
	hidden:true,
	iconCls : 'icon-search',
	handler : function() {
		searchOrgnewMoveout2();
	}
});

var btn_refresh_orgnewMoveout2 = new Ext.Button({
	text : '刷新',
	iconCls : 'icon-refresh',
	handler : function() {
		textSearchOrgnewMoveout2.value='';
		searchOrgnewMoveout2();
	}
});

var text_search_orgnewMoveout2 = new Ext.form.TextField({
	id : 'textSearchOrgnewMoveout2',
	hidden:true,
	name : 'textSearchOrgnewMoveout2',
	width : 200,
	emptyText : '多条件可用逗号或者空格隔开!',
	listeners : {
		'specialkey' : function(field, e) {
...
//默认查询 limit为显示的条数
var searchOrgnewMoveout2 = function() {
	ds_orgnewMoveout2.baseParams.conditions = text_search_orgnewMoveout2.getValue();
	ds_orgnewMoveout2.baseParams.username=currentUsername;
	ds_orgnewMoveout2.baseParams.stateConditions='60,61,62,63,64';
	ds_orgnewMoveout2.load({params : {start : 0,limit : 10} });
};


//---------------------- 列表显示主要数据信息 ----------------------------------
//列表显示机构的主要信息 yangqi
var sm61 = new Ext.grid.CheckboxSelectionModel();
var cm_orgnew61 = new Ext.grid.ColumnModel([expander_imo2,
	sm61,
	{header : '原文',width : 20,dataIndex : 'imageUrl', renderer:icon,sortable : false},
	{header : '流水号',width : 55,dataIndex : 'orderid',sortable : true}, 
	{header : '机构名称',width : 115,sortable : true,dataIndex : 'jgmc'},
...
//加载右栏主页面   （数据列表、工具栏按钮）
var grid_LicenseMoveout2 = new Ext.grid.GridPanel({
	region : 'center',
	loadMask : {msg : '数据加载中...'},
	enableColumnMove : false,
	id:'grid_LicenseMoveout2',
	//autoHeight : true,
	cm : cm_orgnew61,
	ds : ds_orgnewMoveout2,
	sm : new Ext.grid.RowSelectionModel({singleSelect : true}),
	autoExpandColumn : 'address2',
	
	viewConfig : {forceFit : true},
	plugins : expander_imo2,
	tbar : [btn_refresh_orgnewMoveout2,btn_print_orgnewMoveout2,'->',text_search_orgnewMoveout2,
	        btn_search_orgnewMoveout2],
	bbar : new Ext.PagingToolbar({
		pageSize : 10,
		store : ds_orgnewMoveout2,
		displayInfo : true,
		displayMsg : '第 {0} - {1} 条 共 {2} 条',
		emptyMsg : "没有工单记录"
	}),
	listeners : {
		'rowclick':function(grid,rowIndex){		
			if(grid.getStore().getAt(rowIndex).data.state == 62){
			    btn_print_orgnewMoveout2.setDisabled(false);
			   // btn_print_orgnewMoveout3.setDisabled(false);
			}
		}
	}
});
...
var MoveoutPtForm = {
	    ...
	    id: 'Moveout5',
	    border:false,
	    items: [grid_LicenseMoveout2]   
};



var p_useLicenseMoveout = {
	id: 'useLicenseMoveout-panel',
	iconCls : 'icon-plugin',
	layout : 'border',
	title: '迁出申请',
	activeTab: 0,
	items:[{ 
		...
		/*
		{
			...
			listeners : {
				'activate' : function() {  //标签激活后自动加载数据
					text_search_orgnewMoveout2.value='';
					searchOrgnewMoveout2();
				}
			}
		},
		*/
		{
			...
			listeners : {
				'activate' : function() {  //标签激活后自动加载数据
					...
					//上传文件格式限制串
					var strPageTypeNeed="";
					var resultObject826 = null;
					Ext.Ajax.request({  
						 url: 'findByCategoryid.action?categoryId=32',  
						 method:'POST',  
						 success:function(result,request){
							  //alert(result.responseText);
							  resultObject826 = eval('('+result.responseText+')');  
				    		  //Ext.Msg.alert('提示框',resultObject826.remark); 
							  strPageTypeNeed=resultObject826.root[0].remark;
							  Ext.Ajax.request({  
									 url: 'findOrgidOrgnew.action',
									 params : {orgid : currentOrgid,stateConditions:'100,10,13'},
									 method:'POST',  
									 success:function(result,request){
										var resultObject8611 = Ext.util.JSON.decode(result.responseText);
					   					if(resultObject8611.root.length!=0){
					   						var strTbrsfzh=resultObject8611.root[0].tbrsfzh;
					   						var strZjhm=resultObject8611.root[0].zjhm;
					   						//alert(strTbrsfzh+"|"+strZjhm);
					   						if(strTbrsfzh==strZjhm){
					   							var strPageTypeNeed2=strPageTypeNeed.replace(/\经办人身份证件%/g, "%"); 
					   							scanner86.PageNeed=strPageTypeNeed2;
					   							//alert("===="+strPageTypeNeed2)
					   						}else{
					   							scanner86.PageNeed=strPageTypeNeed;
	        		...
	        		var resultObject86 = null;
	        		Ext.Ajax.request({
						url : 'orgnewViewImg.action',
						params : {orgid : strOrgid,stateConditions:'100,60,63'},
						success : function(result,request) {//获取返回值
		   					var resultObject86 = Ext.util.JSON.decode(result.responseText);
		   					if(resultObject86.root.length!=0){
		   						if(resultObject86.root[0].state!='100'){
				   					if(resultObject86.root[0].imageData!=null && resultObject86.root[0].imageData!=""){
				   						scanner86.OpenProgress(3);  //设置上传的进度条的总进度数
				   						scanner86.Progress('原文加载中',1);
				   						scanner86.Progress('原文加载中',2);
				   						scanner86.ImageData=resultObject86.root[0].imageData;
				   						if(resultObject86.root[0].PageType!=""){
				   							scanner86.PageType=resultObject86.root[0].pageTypeStr;
				   						}
				   						scanner86.Progress('原文加载完毕',3);
				   						scanner86.CloseProgress();
				   					}
								}
		   					}
			...
			listeners : {
				'activate' : function() {  //标签激活后自动加载数据
					text_search_orgnewMoveout.value='';
					searchOrgnewMoveout();
		...
		listeners : {
			'tabchange' : function() {
			   //alert('成功'+userstate+'吗！')
				switch (userstate) {  //设置迁出状态 ?????此处应该采用机构的条目吧？
			 	case '7' :
				...
				var tab61=Ext.getCmp("centerPanel7");
				if(userstate=="62"||userstate=="65"){
					tab61.remove("MoveoutTab");   //移除标签
					tab61.remove("MoveoutScTab");   //移除标签
					//tab61.remove("MoveoutPtTab");   //移除标签
